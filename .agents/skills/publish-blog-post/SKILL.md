...
description: "Manually prepare one posts/*.md source as blog/*.html and update blog/index.html as an emergency fallback. Use only when explicitly invoked and build_blog.py cannot run; use homepage-build for normal generation."
---
...
This skill prepares local output; it does not authorize a commit, push, or deployment.
...
6. Keep index rows limited to title and date, with `data-category` for filtering. Render `post-tags` / `post-tag` badges in the individual post header.
7. Update category counts or filter labels if the new post changes them.
8. Include the canonical, Open Graph, and Twitter metadata generated by `social_metadata.py`, using the new post's own URL, title, date, and description.
...
- If tags exist in front matter, keep their order stable in the post header; do not add tag badges to index rows.
...
- run `python3 .agents/skills/homepage-build/scripts/check_posts.py --generated` and `python3 scripts/check_social_metadata.py` when the environment permits; report checks that cannot run
- record a full rebuild and comparison as pending until the build environment is restored
