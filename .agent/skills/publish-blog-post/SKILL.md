...
2. Determine the post title, date, category, tags, and slug from the file content or filename.
3. Create or update the matching `blog/<slug>.html` file using the shared blog layout.
4. Add the post to `blog/index.html` in the correct year section and date order.
5. Render tag badges next to the title in the index when tags are available, using the same `post-tags` / `post-tag` structure as existing rows.
6. Update category counts or filter labels if the new post changes them.
